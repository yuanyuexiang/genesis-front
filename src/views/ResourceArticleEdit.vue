...
export default {
  ...
  methods: {
    ...
    submitArticle() {
      updateArticle(this.articleForm.id,this.articleForm)
        .then(response => {
  ...
  mounted() {
    this.listAllMedia(this.offset);
    this.articleForm  = this.$store.getters.postoffice_data;
    console.log("------------------------------------------");
    console.log(this.$store.getters.postoffice_data);
    this.fileUrl = this.articleForm.thumb_url;
  },
};
</script>
